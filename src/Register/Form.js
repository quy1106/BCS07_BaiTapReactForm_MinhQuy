...
class Form extends Component {
  ...
  handleSubmit = (e) => {
    e.preventDefault();
    const validateId = (id) => {
      const idValidate = this.props.arrRegisterReducer.findIndex(
        (item) => item.id === id
      );
      if (idValidate !== -1) {
    ...
    const { nameError, emailError, phoneError, idError } =
      this.props.arrErrorReducer;
  ...
  render() {
    ...
    const { nameError, emailError, phoneError, idError } =
      this.props.arrErrorReducer;
    const { idInputState, addButtonState, editButtonState } =
      this.props.stateFormReducer;
    return (
      <div className="container">
        <div className="card ">
          <div className="card-header bg-dark text-white">
            <p className="fs-3 text-start fw-fw-semibold">
              Thông tin sinh viên
            </p>
          </div>
              ...
              <div className="row">
                <div className="form-group col-sm-12 col-md-6  mt-3">
                  <div className="d-flex justify-content-between">
                    ...
                    <p
                      className="error_input text-end text-danger"
                      id="error_id"
                    >
                      {idError}
                    </p>
